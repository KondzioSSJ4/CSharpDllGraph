...
## [x] Task 1: Load Roslyn workspace and compilations
...
- [x] 1.1 Add `Microsoft.CodeAnalysis.Workspaces.MSBuild`
- [x] 1.2 Register MSBuild locator once during engine startup
- [x] 1.3 Open the solution and cache `Compilation` plus `SemanticModel` per relevant document
- [x] 1.4 Surface compile diagnostics as warnings instead of aborting the graph build

## [x] Task 2: Resolve symbol usages into graph edges
...
- [x] 2.1 Walk invocation expressions, member access, object creation, attributes, and type references
- [x] 2.2 Resolve each symbol to its containing assembly, package, and package version using the phase-02 provider outputs
- [x] 2.3 Emit `Calls` for method invocations and `Uses` for non-call type references
- [x] 2.4 Deduplicate edges per `(from, to, kind)` and aggregate call count metadata

## [x] Task 3: Emit user-code nodes safely
...
- [x] 3.1 Emit user `Type` and `Method` nodes with `origin=user`
- [x] 3.2 Use a stable project-specific version token for user nodes
- [x] 3.3 Skip generated files through Roslyn and common generated-code markers
- [x] 3.4 Reuse source span tracking so returned call sites can later power MCP tools

## [x] Task 4: Prove usage edges with integration fixtures
...
- [x] 4.1 Extend the sample solution with user code that calls fixture package APIs
- [x] 4.2 Run the provider end to end and persist the graph to a temp workspace
- [x] 4.3 Assert expected `Calls` and `Uses` edges with correct target version tags
- [x] 4.4 Assert no cross-version leakage when user code references only one package version

## [x] Task 5: Pass the validate-stop gate
...
- [x] 5.1 Prove usage edges exist for every relevant referenced public symbol in the fixture
- [x] 5.2 Prove target version tags are correct
- [x] 5.3 Prove integration tests are green
- [x] 5.4 Print a signed summary and halt for human sign-off
...
- [x] 6.1 Review `docs/PROJECT_DEFINITION.md` against new usage-edge behavior
- [x] 6.2 Update touched phase-03 requirements in `docs/REQUIREMENT_DEFINITION.md`
